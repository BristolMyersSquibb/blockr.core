...
board_server.board <- function(id, x, plugins = board_plugins(x),
  ...
  log_info(
    "[RELOAD-DEBUG] board_server.board | ",
    "reload_meta: {if (!is.null(reload_meta)) coal(reload_meta$url, '(no url)') else '(null)'}"
  )
      ...
      if (not_null(board_refresh)) {
        ...
        observe({
          ...
          log_info(
            ...
            "pending_age: {round(difftime(Sys.time(), pending, units = 'secs'), 1)}s"
          )
          ...
          log_info(
            "[RELOAD-DEBUG] calling update_serve_obj + session$reload | ",
            "session: {substr(session$token, 1, 8)} | ",
