...
render_mermaid <- get("render_mermaid", envir = asNamespace("blockr.core"))
```
...
```{r, echo=FALSE, include=FALSE, eval=!nzchar(Sys.getenv("CI"))}
render_mermaid("../man/diagrams/block-structure.mmd")
```
...
```{r, echo=FALSE, include=FALSE, eval=!nzchar(Sys.getenv("CI"))}
render_mermaid("../man/diagrams/block-inputs.mmd")
```
