...
- Board files are now written and read by `typedjson` rather than
  ...
  one number type, no typed `NA` and no place for attributes, so a
  double came back an integer, an all-`NA` vector lost its type, and
  names, `Date` and factor classes were dropped. Core’s own objects were
  unaffected because
  [`blockr_deser()`](https://bristolmyerssquibb.github.io/blockr.core/reference/blockr_ser.md)
  rebuilds them through a constructor that re-imposes the types the
  document could not carry, but anything an extension stored in `state`
  has no such route back and arrived changed. Both directions of the
  format change were measured: a document written by the old pair reads
...
- A link input equal to `"Inf"`, `"NA"` or `"NaN"` now restores as the
  ...
  field came back non-character and aborted the restore. Strings are now
