...
* Core's own board UI can now drive visibility, through a board callback like
  any other front-end rather than from inside the board server. Stacks render
  as an accordion which opens one stack and collapses the rest, so on a stacked
  board part of what is on screen was hidden from the first render while every
  block evaluated and rendered regardless -- nothing read the input `bslib` had
  ...
  rebuild. Which stack starts open is derived server-side rather than awaited
  from the client, since core renders it. It is `board_server()`'s default
  `callbacks` value, so a board driven by another front-end -- which passes its
  own callbacks -- leaves core tracking nothing; a consumer that wants both
  keeps it in the list, `callbacks = list(gate_stacks(), my_callback)`. Whether
  it gates is the `gate_stacks` [blockr_option()], `FALSE` by default because
  turning it on is a behaviour change: expanding a stack starts the computation
  rather than revealing a finished one. The accordion container ID moves from
  `<board>_stacks` to the board-namespaced `<board>-stacks`, which is what
